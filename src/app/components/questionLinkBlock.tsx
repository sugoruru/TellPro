    ...
    <>
      <div className="border-gray-200">
        <div className="bg-white transition border-b relative max-w-[60rem] mt-3 min-h-40 rounded-lg break-words mx-auto">
          <Link href={`/${props.question.userID}/questions/${props.question.ID}`} prefetch className="min-h-40 block">
            <div className="flex p-5">
              <div>
                <div className="flex">
                  <Image alt={props.questionUser.username} src={props.questionUser.icon} width={24} height={24} priority />
                  <u
                    className="ml-1 cursor-pointer"
                    onClick={() => {
                      router.push(`/${props.question.userID}`);
                    }}
                  >
                    @{props.question.userID}
                  </u>
                </div>
                <b className="mr-1">{props.question.title}</b>
                <div className={`${props.question.isPublic ? "bg-blue-400" : "bg-red-400"} text-white px-1 rounded-sm inline-block mb-1`}>{props.question.isPublic ? "公開" : "非公開"}</div>
          ...
          {!props.stateFunctions ? (
            ...
            <div className={`${props.me?.ID === props.question.userID ? "" : "hidden"} absolute top-3 right-3`}>
              <div className="flex">
                <Link title="編集" className={`cursor-pointer z-10`} href={`/${props.question.userID}/questions/${props.question.ID}/edit`}>
                            ...
                            <button
                              onClick={() => {
                                if (props.stateFunctions) {
                                  ...
                                  props.stateFunctions.setDeletePageID(props.question.ID);
                                }
                              }}
...
export default QuestionLinkBlock;
