...
export async function GET(req: NextRequest) {
    ...
    try {
      ...
      if (data.length > 0) {
        if (data[0].ID === req.nextUrl.searchParams.get("userID")) {
          // 非公開のページも取得.
          const pages = await db.any(`SELECT ${pageBlockKey} FROM "Questions" WHERE "userID" = $1`, [req.nextUrl.searchParams.get("userID")]);
  ...
  // 公開ページのみ取得.
  const pages = await db.any(`SELECT ${pageBlockKey} FROM "Questions" WHERE "userID" = $1 AND "isPublic" = true`, [req.nextUrl.searchParams.get("userID")]);
  const res = NextResponse.json({ ok: true, pages: pages }, { status: 200 });
  return res;
}
